...
void ConvexMpcLocoGaits::SetDefaultParams() {
    ...
    rpy_cmd_scale_ << 1.0, 1.0, 0.0;  // only pitch is enabled
    ...
    vel_cmd_zero_offset_.setZero();
    rpy_cmd_min_ << -0.47, -0.1, 0.0;  // only pitch is enabled
    rpy_cmd_max_ << 0.47, 0.1, 0.0;
...
void ConvexMpcLocoGaits::SetPassiveTrotParams() {
    ...
    rpy_cmd_min_ << -0.47, 0.0, 0.0;  // rpy_cmd_[2] is zero
    rpy_cmd_max_ << 0.47, 0.0, 0.0;
...
void ConvexMpcLocoGaits::SetCmd( ControlFsmData< float >& data ) {
    ...
    // set rpy_des_
    SetYawDesire();
    rpy_cmd_[ 0 ] = ctrl_cmd_->rpy_des[ 0 ];  
    rpy_des_( 0 ) = WrapRange( rpy_cmd_[ 0 ], rpy_cmd_scale_( 0 ) * rpy_cmd_min_( 0 ), rpy_cmd_scale_( 0 ) * rpy_cmd_max_( 0 ) );
