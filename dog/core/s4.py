...
障碍物绕行方向：本文件 DETOUR_SIDE=+1，从障碍物右侧绕；从左侧绕的镜像版见 stage4_real_left.py。
...
运行（电脑识别服务 + 狗上主程序）：
  狗①: source /etc/mi/ros2_env.conf
       python3 /home/mi/cyberdog_competition/stage4_real_right.py --start-at-corridor
       （主程序默认自动启动 RGB 推流）
...
STAGE4_FINISH_X = 3.250
STAGE4_ENTRY_Y = 6.580
RIGHT_START_LINE_TARGET = 0.30
RIGHT_FINISH_LINE_TARGET = 0.23
CORRIDOR_LINE_TARGET = TARGET_LINE
THIRD_CHANNEL_LEFT_LINE_TARGET = 0.50
THIRD_CHANNEL_ENTRY_CAL_X = 0.12
THIRD_CHANNEL_ENTRY_LINE_FOLLOW_M = 0.50
...
DETOUR_SIDE = 1                        # 障碍绕行方向：+1=从右侧绕；-1=从左侧绕（镜像版见 stage4_real_left.py）
...
class Stage4Real(object):
    ...
    def calibrate_third_channel_return(self):
        ...
        x, y, _z = self._pos()
        print("[stage4] 第三通道左线校准 %s：线距=%.2fm，坐标保持 x=%.3f y=%.3f yaw=%.1f" %
              ("完成" if ok else "未达标",
               THIRD_CHANNEL_LEFT_LINE_TARGET, self._stage_x(), self._stage_y(), self._yaw()),
              flush=True)
        return ok

    def calibrate_third_channel_entry(self):
        """第三通道入口面向 +y，用左鱼眼校到中心后写回绝对 x=0。"""
        print("[stage4] 第三通道入口：x≈%.2f，左鱼眼校准左线距=%.2fm" %
              (THIRD_CHANNEL_ENTRY_CAL_X, THIRD_CHANNEL_LEFT_LINE_TARGET), flush=True)
        if not self.adp.align_yaw(FWD_YAW, tol=2.0, timeout=8.0):
            print("[stage4] 第三通道入口转回 +y 失败，跳过左线校准", flush=True)
            return False
        ok = self.line.end_calibrate(
            self.adp,
            side="left",
        ...
        if ok:
            print("[stage4] 第三通道入口：左鱼眼闭环沿 +y 前进 %.2fm" %
                  THIRD_CHANNEL_ENTRY_LINE_FOLLOW_M, flush=True)
            ok = self.line.line_follow_fwd_distance(
                self.adp,
                side="left",
                distance=THIRD_CHANNEL_ENTRY_LINE_FOLLOW_M,
                target_dist=THIRD_CHANNEL_LEFT_LINE_TARGET,
                stop_callback=self._third_channel_entry_stop_reason,
            )
        if ok and not self.adp.align_yaw(FWD_YAW, tol=2.0, timeout=8.0):
            print("[stage4] 第三通道入口前向对齐失败，不写回 x=0", flush=True)
            ok = False
        if ok:
            pos_y, _pos_x, _z = self._pos()
            self.adp.set_mapped_pose(
                pos_y,
                STAGE4_START_X - CHANNEL_X[-1],
                self._yaw(),
            )
            print("[stage4] 第三通道入口左线中心已写回绝对 x=%.3f" %
                  CHANNEL_X[-1], flush=True)
        print("[stage4] 第三通道入口左线校准 %s：线距=%.2fm，当前 x=%.3f y=%.3f yaw=%.1f" %
              ("完成" if ok else "未达标",
               THIRD_CHANNEL_LEFT_LINE_TARGET, self._stage_x(), self._stage_y(), self._yaw()),
              flush=True)
        return ok

    def _third_channel_entry_stop_reason(self):
        """第三通道入口循线时，给障碍物动作链及时让路。"""
        self._wait_detector()
        detections = self.det.get_detections()
        if any(d.get("class_name") == "obstacle" for d in detections):
            return "检测到不可跨越障碍物"
        bar_norms = [d.get("bbox_area_norm", 0.0) for d in detections
                     if d.get("class_name") == "limit_bar"]
        if bar_norms and max(bar_norms) >= self.bar_norm:
            return "限高杆已到低姿阈值 %.3f" % max(bar_norms)
        return None
    ...
    def traverse_channel(self, channel_x, lane_x=None):
        if lane_x is None:
            lane_x = channel_x
        self._pending_obstacle2 = None
        self._set_speech_mute("up")
        print("[stage4] ===== 通道 x=%.3f 上行（保持 x=%.3f） =====" %
              (channel_x, lane_x))
        cls1 = self.walk_until_detected(OBSTACLE1, vx=WALK_VX, going_up=True, lock_x=lane_x)
        self._stop()
        ...
        if cls1 == "limit_bar":
            self._low_on(vx=LOW_VX)
            low_seg = self.handle_limit_bar(lock_x=lane_x)
        elif cls1 == "obstacle":
            self.handle_obstacle(lane_x, going_up=True)
        ...
        if self._pending_obstacle2 is not None:
            ...
            cls2 = self.walk_until_detected(OBSTACLE2, vx=WALK_VX, going_up=True, lock_x=lane_x)
        self._stop()
        if cls2 == "football":
        ...
        if cls1 == "obstacle":
            print("[stage4] 下行绕障碍")
            self.walk_to_y(BB_DN_START_Y, vx=-WALK_VX, lock_x=lane_x)
            self.handle_obstacle(lane_x, going_up=False)
        elif cls1 == "limit_bar" and low_seg is not None:
            self.handle_limit_bar_down(lock_x=lane_x,
                                       low_start_ref=low_seg[0], low_end_ref=low_seg[1])

        print("[stage4] 回到走廊 y=%.3f" % CORRIDOR_Y)
        self.walk_to_y(CORRIDOR_Y, vx=-WALK_VX, lock_x=lane_x)
    ...
    def run(self, max_channels=None):
        ...
        for i, channel_x in enumerate(channels, 1):
            ...
            if i == 1:
            ...
            elif abs(channel_x - CHANNEL_X[-1]) < 1e-6:
                print("[stage4] 前往通道 %d x=%.3f：先循线到 x=%.2f，再面向 +y 左鱼眼校准" %
                      (i, channel_x, THIRD_CHANNEL_ENTRY_CAL_X))
                if self.line_ready():
                    ok = self.line_transition(
                        side="left",
                        stop_x=THIRD_CHANNEL_ENTRY_CAL_X,
                        target_yaw=90.0,
                    )
                    if ok:
                        if self.calibrate_third_channel_entry():
                            lane_x = channel_x
                            print("[stage4] 第三通道入口采用绝对通道保持 x=%.3f" % lane_x,
                                  flush=True)
                    else:
                        print("[stage4] 第三通道入口循线失败，回退斜走")
                        self.walk_to_xy(CORRIDOR_Y, channel_x)
                else:
                    self.walk_to_xy(CORRIDOR_Y, channel_x)
            else:
                ...
                if self.line_ready():
                    ...
                    self.walk_to_xy(CORRIDOR_Y, channel_x)
            self.traverse_channel(channel_x, lane_x=lane_x)
